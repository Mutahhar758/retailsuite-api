...
namespace Retailer.Infrastructure.Legacy.Payments;

internal class PaymentService : IPaymentService
{
    private const string VType = "PV";

    private readonly IRepository<GlEntry> _glRepository;

    public PaymentService(IRepository<GlEntry> glRepository)
    {
        _glRepository = glRepository;
    }

    public async Task<List<PaymentResponse>> GetListAsync(PaymentListFilter filter, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(filter.CashBankAccount))
            query = query.Where(x => x.CrAccountId == filter.CashBankAccount);

        if (!string.IsNullOrWhiteSpace(filter.Account))
            query = query.Where(x => x.DrAccountId == filter.Account);
    ...
    public async Task<List<PaymentLineResponse>> GetDetailAsync(string voucherNo, string? cashBankAccount, CancellationToken cancellationToken)
    {
        var query = _glRepository.GetAll()
            .AsNoTracking()
        ...
        return await query
            .OrderBy(x => x.VSeq)
            .Select(x => new PaymentLineResponse
            {
                Seq = x.VSeq,
                Date = x.VDate,
    ...
    public async Task<string> CreateAsync(PaymentCreateRequest request, CancellationToken cancellationToken)
    {
        ...
        foreach (var line in request.Lines)
        {
            await _glRepository.AddAsync(new GlEntry
            {
                ...
                VType = VType,
                VSeq = line.Seq,
                DrAccountId = line.Account,
                Amount = line.Amount,
                CrAccountId = request.CashBankAccount,
    ...
    public async Task UpdateAsync(string voucherNo, PaymentUpdateRequest request, CancellationToken cancellationToken)
    {
        var anyExists = await _glRepository.GetAll()
            .AnyAsync(x => x.VType == VType && x.VoucherNo == voucherNo, cancellationToken);

        if (!anyExists)
            throw new NotFoundException($"Payment voucher '{voucherNo}' not found.");
        ...
        foreach (var line in request.Lines)
        {
            ...
            else
            {
                ...
                existing.CrAccountId = request.CashBankAccount;
