...
{
    private readonly ProfitByItemReportHeader _header;
    private readonly List<ProfitByItemReportItem> _items;

    public ProfitByItemDocument(
        ProfitByItemReportHeader header,
        List<ProfitByItemReportItem> items)
    {
        _header = header ?? new ProfitByItemReportHeader();
        ...
        container.Column(col =>
        {
            col.Item().Row(row =>
            {
                row.RelativeItem().Column(brandCol =>
                {
                    ...
                    brandCol.Item().PaddingTop(2).Text($"Period: {_header.FromDate:dd-MMM-yyyy} to {_header.ToDate:dd-MMM-yyyy}" + (!string.IsNullOrWhiteSpace(_header.ItemFilter) ? $"  |  Item: {_header.ItemFilter}" : string.Empty) + (!string.IsNullOrWhiteSpace(_header.CategoryFilter) ? $"  |  Category: {_header.CategoryFilter}" : string.Empty))
                        .FontSize(9)
    ...
    private void ComposeContent(IContainer container)
    {
        container.PaddingTop(6).Table(table =>
        {
            ...
            for (int i = 0; i < _items.Count; i++)
            {
                ...
                table.Cell().Element(c => CellStyle(c, bg)).Text(item.ItemId);
                table.Cell().Element(c => CellStyle(c, bg)).Text(item.ItemTitle).SemiBold();
                ...
                table.Cell().Element(c => CellStyle(c, bg)).AlignRight().Text($"{item.AvgSaleRate:#,##0.00}");
                table.Cell().Element(c => CellStyle(c, bg)).AlignRight().Text($"{item.TotalSales:#,##0.00}");
                table.Cell().Element(c => CellStyle(c, bg)).AlignRight().Text($"{item.AvgCostRate:#,##0.00}");
