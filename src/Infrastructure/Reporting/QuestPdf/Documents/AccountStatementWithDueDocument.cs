...
/// <summary>
/// Code-first QuestPDF document implementing an executive, modern Account Statement With Due Days & Due Date.
/// Follows institutional corporate accounting and credit control reporting standards.
/// </summary>
public class AccountStatementWithDueDocument : IDocument
{
    private readonly AccountStatementWithDueHeader _header;
    private readonly List<AccountStatementWithDueReportItem> _items;

    public AccountStatementWithDueDocument(AccountStatementWithDueHeader header, List<AccountStatementWithDueReportItem> items)
    {
        _header = header ?? new AccountStatementWithDueHeader();
        _items = items ?? new List<AccountStatementWithDueReportItem>();
    }
        ...
        container.Column(col =>
        {
            ...
            col.Item().Row(row =>
            {
                row.RelativeItem().Column(brandCol =>
                {
                    ...
                    brandCol.Item().PaddingTop(2).Text("ACCOUNT STATEMENT (WITH DUE DAYS)")
                        .FontSize(11)
                        .SemiBold()
    ...
    private void ComposeContent(IContainer container)
    {
        container.PaddingTop(8).Table(table =>
        {
            table.ColumnsDefinition(columns =>
            {
                ...
                columns.ConstantColumn(54);  // Voucher No
                columns.RelativeColumn(2.8f); // Particulars
                columns.ConstantColumn(46);  // Due Days
            ...
            table.Header(header =>
            {
                ...
                header.Cell().Element(HeaderCell).AlignCenter().Text("Due Days");
                header.Cell().Element(HeaderCell).AlignCenter().Text("Due Date");
            ...
            for (int i = 0; i < _items.Count; i++)
            {
                ...
                // Due Days
                if (item.DueDays.HasValue && item.DueDays.Value > 0)
                    table.Cell().Element(c => BodyCell(c, bg)).AlignCenter().Text(item.DueDays.Value.ToString() + " d");
                else
                    table.Cell().Element(c => BodyCell(c, bg)).AlignCenter().Text("-").FontColor(Colors.Grey.Lighten1);

                // Due Date
                if (item.DueDate.HasValue)
                    table.Cell().Element(c => BodyCell(c, bg)).AlignCenter().Text(item.DueDate.Value.ToString("dd-MMM-yyyy"));
                else
                    table.Cell().Element(c => BodyCell(c, bg)).AlignCenter().Text("-").FontColor(Colors.Grey.Lighten1);
            ...
            table.Cell().ColumnSpan(5).Element(TotalCell).Text("TOTALS & NET MOVEMENT").Bold();
