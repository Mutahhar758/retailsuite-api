    ...
    internal static IServiceCollection AddOpenApiDocumentation(this IServiceCollection services, IConfiguration config)
    {
        ...
        if (settings.Enable)
        {
            ...
            _ = services.AddOpenApiDocument((document, serviceProvider) =>
            {
                ...
                document.OperationProcessors.Add(new TenantIdHeaderOperationProcessor());
    ...
    internal static IApplicationBuilder UseOpenApiDocumentation(this IApplicationBuilder app, IConfiguration config)
    {
        if (config.GetValue<bool>("SwaggerSettings:Enable"))
        {
            app.UseOpenApi(o =>
            {
                ...
                {
                    //var url = doc.Servers.FirstOrDefault().Url;
                    doc.Servers.Clear();
                    // doc.Servers.Add(new OpenApiServer { Url = url.Replace("http://", "https://") });  //... add server
                };
            });
            app.UseSwaggerUi(options =>
            {
                ...
                options.CustomJavaScriptPath = OpenApiConstants.CustomJavaScriptPath;
